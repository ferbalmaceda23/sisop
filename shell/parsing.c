...
expand_environ_var(char *arg)
{
	if (arg[0] == '$') {
		if (strcmp(arg, "$?") == 0) {
			char string_status[PRMTLEN];
			snprintf(string_status, PRMTLEN, "%d", status);
			strcpy(arg, string_status);
		} else {
			...
			if (value == NULL || value_size == 0) {
				...
				if (value_size > sizeof(arg)) {
					char *arg_aux = realloc(arg, value_size);
					if (arg_aux) {
						strcpy(arg_aux, value);
						arg = arg_aux;
					}
				} else {
					strcpy(arg, value);
...
parse_exec(char *buf_cmd)
{
	...
	while (buf_cmd[idx] != END_STRING) {
		...
		// code changed to handle the case when the env var is empty
		if (strlen(tok) > 0)
			c->argv[argc++] = tok;
	}
...
parse_line(char *buf)
{
	...
	// code changed to handle the case when there's multiple pipes
	if (strlen(right) > 0)
		r = parse_line(right);
	else
		r = parse_cmd(right);
