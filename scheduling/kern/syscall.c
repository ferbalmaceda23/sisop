...
// Change the priority of a given environment.
//
// Returns 0 on success, < 0 on error.  Errors are:
//	-E_INVAL if priority is bigger than maximum or better than current priority
static int
sys_changepriority(uint32_t priority)
{
	if (priority < 0 || priority > MAX_ENV_PRIORITY)
		return -E_INVAL;
